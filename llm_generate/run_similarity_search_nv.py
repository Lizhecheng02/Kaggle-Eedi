...
if args.num_beam != 1:
    ...
    for idx in range(len(preds_vectors)):
        ...
        preds_cos_sim_arr = np.mean(preds_cos_sim_arr, axis=0).reshape(1,-1)
        ...
        t_score = []
        for i in preds_sorted_indices[0]:
            # print(preds_cos_sim_arr[0].shape)
            tmp = preds_cos_sim_arr[0].tolist()
            t_score.append(tmp[i])
    ...
    for i, preds in enumerate(preds_sorted_indices):
        t_score = []
        for idx in preds:
            t_score.append(preds_cos_sim_arr[i, idx])
        scores.append(t_score)
...
submission = result_df[["QuestionId_Answer", "MisconceptionId", 'scores']]
