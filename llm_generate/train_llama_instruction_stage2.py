...
os.environ['WANDB_API_KEY'] = "115a6bce1359c3feb6602f9f65a2ff034845669e"
...
class InstructionDataSet(Dataset):
    ...
    def prompt_fun1(self, now_data):
        ...
        prompt1 = prompt1.format('#most likely 25 misconceptions:\n' + now_data['MisconceptionText'])

        prompt2 = now_data['prompt_response']
        prompt_response = prompt_text_1.format(prompt1 + prompt2 + "\n" + prompt2 + "\n" + prompt2)
        return prompt_response
    

    def prompt_fun2(self, now_data):
        '''选择一个'''
        prompt_text_1 = "<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n{}<|eot_id|>\n<|start_header_id|>assistant<|end_header_id|>"
        # prompt_text_2 = "<|start_header_id|>assistant<|end_header_id|>"
        prompt1 = """
Here are students answer Diagnostic Questions (DQs), which are choice questions featuring one correct answer and one incorrect answers, known as distractors.
Each question targets a specific construct (also referred to as a skill), representing the most granular level of knowledge relevant to the question.
Your task: According to the following Incorrect answer.
...
{prompt2}
"""     
        reponse = now_data['prompt_response']
        
        prompt2 = """Here are the 25 most likely misconceptions. Identify the most appropriate misconceptions behind the incorrect answers. \n#{}\n\n"""
        prompt2 = prompt2.format('misconceptions:\n' + now_data['MisconceptionText'])
        prompt_response = prompt1.format(
            text=reponse + "\n" + reponse + "\n" + reponse,
            prompt2=prompt2
        )
        prompt_response = prompt_text_1.format(prompt_response)
    ...
    def __getitem__(self, index):
        ...
        output = now_data['MisconceptionName']
        prompt_response = self.prompt_fun1(now_data)
        label = output
        ...
        label_ids = self.tokenizer.encode(text=label, add_special_tokens=False, truncation=True,
                                            max_length=self.max_target_length, padding=False)
...
def train(args):
    ...
    wandb.init(project="Eedi", config=args)
    wandb.save("train_llama_instruction_stage2.py")
    ...
    if args.use_cache and os.path.exists(train_cache_path):
        ...
        tokenized_dataset = InstructionDataSet(df_train,tokenizer, args.MAX_INPUT, args.MAX_OUTPUT)
    ...
    training_args = TrainingArguments(
            ...
            output_dir=f"/home/chenning/code/Eedi/output/{wandb.run.name}",
            report_to="wandb",
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', default='/home/chenning/code/Eedi/src/config/train_llama_instruction_stage2.yaml', type=str, help='Path to the config file', required=False)
