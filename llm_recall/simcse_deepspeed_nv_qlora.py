...
class TrainDatasetForEmbedding(Dataset):
    def __init__(
        ...
        self.query_instruction_for_retrieval = "Instruct: Your task is to retrieve the possible misconception that might lead students to choose the incorrect answer in Mathematics multiple-choice questions.\n Query: "
    ...
    def __getitem__(self, item) -> Tuple[str, List[str]]:
        ...
        if self.passage_instruction_for_retrieval is not None:
            passages = [self.passage_instruction_for_retrieval + p for p in passages]
        # print("pos:", pos)
...
class BiEncoderModel(nn.Module):
    def __init__(self,
        ...
        model = AutoModel.from_pretrained(model_path, trust_remote_code=True, config=config, quantization_config=bnb_config)
        max_length = 32768
    ...
    def encode(self, features):
        ...
        p_reps = psg_out['sentence_embeddings']
        # p_reps = self.sentence_embedding(psg_out.last_hidden_state, features['attention_mask'])
        # print(psg_out['sentence_embeddings'].shape)
        # p_reps = self.sentence_embedding(psg_out['sentence_embeddings'], features['attention_mask'])
        if self.normlized:
    ...
    def forward(self, query, passage):
        # print(query['input_ids'].shape, passage['input_ids'].shape)
        q_reps = self.encode(query)
        p_reps = self.encode(passage)
        # print(q_reps.shape, p_reps.shape)
        # print(self.negatives_cross_device,self.use_inbatch_neg )
        if self.training:
...
def main():
    ...
    args = parse_args()
    # args.train_data = '/home/chenning/code/Eedi/recall/datas/finetune_train_minedHN_50.jsonl'
    # args.model_name_or_path = '/home/chenning/opensource_models/nvidia/NV-Embed-v2'
    # args.zero_stage = 1
    ...
    AdamOptimizer = DeepSpeedCPUAdam if args.offload else Adam
